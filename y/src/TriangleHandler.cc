...
Intersection::Intersection(FractureEndContainer_Type const & fractureEnd,
		PointData const & intersectionPoint) :
...
void Intersection::setTriangleIntersection(
	FracturePtrContainer_Type& M_FracturesSet) {
...
nodeI[0] = M_FracturesSet[0]->getMeshFEMVelocity().point_of_basic_dof(
		nbDof0 - 1)[0];
...
if (gmm::abs(
		M_FracturesSet[1]->getLevelSet()->getData()->ylevelSetFunction(node0))
		< 1.0E-2) {
	node0[0] = M_FracturesSet[0]->getMeshFEMPressure().point_of_basic_dof(
			nbDof0 - 1)[0];
	node0[0] = M_FracturesSet[0]->getMeshFEMPressure().point_of_basic_dof(
			nbDof0 - 1)[0];
...
if (gmm::abs(
		M_FracturesSet[0]->getLevelSet()->getData()->ylevelSetFunction(node1))
		< 1.0E-2) {

	node1[0] = M_FracturesSet[1]->getMeshFEMPressure().point_of_basic_dof(
			nbDof1 - 1)[0];
	node1[1] = M_FracturesSet[1]->getLevelSet()->getData()->y_map(tmp1);
}

if (gmm::abs(
		M_FracturesSet[1]->getLevelSet()->getData()->ylevelSetFunction(node2))
		< 1.0E-2) {
	node2[0] = M_FracturesSet[2]->getMeshFEMPressure().point_of_basic_dof(
			nbDof2 - 1)[0];
...
void Intersection::setQuadrilaterIntersection(
	FracturePtrContainer_Type& M_FracturesSet) {
base_node node0(2);
base_node node1(2);
...
for (size_type i = 0; i < 3; ++i) {
	...
	if (std::fabs(nitj) < tol) {
		point = M_intersection
				+ PointData(M_normals[i](0), M_normals[i](1))
						* (0.5
								* (M_fractures[j].getThickness()
										+ M_fractures[i].getThickness()));
	} else {
		// parametric coordinate
		s = 0.5
				* (M_fractures[j].getThickness() * ninj
						+ M_fractures[i].getThickness()) / nitj;

		// the ith point is Pji in the note
		point = M_intersection
				+ (PointData(M_tangents[j](0), M_tangents[j](1)) * s)
				- (PointData(M_normals[j](0), M_normals[j](1))
						* (0.5 * M_fractures[j].getThickness()));
	}
...
bool Intersection::isPos(FractureHandlerPtr_Type& fracture,
	FractureHandlerPtr_Type& otherfracture) {
base_node n0(2);
n0[0] = 0;
...
if (fracture->getLevelSet()->getData()->ylevelSetFunction(n0) >= 0
		&& fracture->getLevelSet()->getData()->ylevelSetFunction(n1) >= 0)
	return true;
else
	return false;

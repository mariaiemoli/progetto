...
namespace Darcy
{
  ...
  void Biforc::computeT(double t)	// vedi definizione pagina 6 paper prof
  {
    //this->computePc();
    this->computeQc();
    this->computeN();
    ...
    T_=(1./area)*( Nkn + t*tmp );		// è giusto dividere per l'area anche il termine t*tmp?
  }
  ...
  {
    double factor = (1.0/T.sum());		// nel paper è s^
    Vector3d tmp;
    tmp(0)=factor*(T.col(0).sum());		// nel paper pI - pigreco= Q/T*vettId

...
void MeshHandler::setUpMesh ( )
{
    if ( M_meshExternal == "none" )
    {
        ...
        length [ 0 ] = M_lengthAbscissa; //LenghtAbscissa == pezzettino di dominio x
        length [ 1 ] = M_lengthOrdinate;
        length [ 2 ] = M_lengthQuota;

        for ( size_type i = 0; i < M_spaceDimension; ++i )
        {
            transformMatrix(i, i) = (i < M_spaceDimension) ? length [ i ] : 1.0; // se vero i < M_spaceDimension metti fractureLength [ i ] altrimenti 1
        }
        if ( M_spaceDimension > 1 )
        {
            transformMatrix(0, 1) = M_inclination * M_lengthOrdinate; //mappa che mi porta dal dominio di riferimento al dominio reale sulle y)
        }
...
void MeshHandler::setUpRegions ( const FracturesSetPtr_Type& fractures )
{
    ...
    for ( i_cv << bv_cv; i_cv != size_type(-1); i_cv << bv_cv ) 
		// << è un operatore di dal::bit_vector -> inline size_t APIDECL &operator << (size_t &i, bit_vector &s) { i = s.take_first(); return i; }
    {
        ...
        for ( size_type f = 0; f < numberFractures; ++f )
        {
            ...
            for ( i_cv << bv_cv; i_cv != size_type(-1); i_cv << bv_cv )
            {
                if ( fractures->getFracture ( f )->getLevelSet()->getMesh().is_convex_cut(
                        i_cv) )
                {
                    M_mesh.region(f + FractureData::FRACTURE).add(i_cv); //enum FRACTURE == 0
                }
            }
...
void MeshHandler::computeMeshMeasures ( )
{
    ...
    /*for ( getfem::mr_visitor vis(M_mesh.convex_index()); !vis.finished(); vis.next() )*/
    ...
    {	
		...
		for ( size_type i = 0; i < M_mesh.nb_faces_of_convex(j); ++i )
        {
            ...
            size_type dof =
					...
					M_meshFEMVector.ind_basic_dof_of_element(j) [ i ];

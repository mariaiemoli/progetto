                  ...
                  const size_type& cutRegionFlag )
{
    ...
    for ( size_type i = 0; i < extendedNumVelocity; ++i )
    {
        ...
        pointMapped[0]= fracture->getLevelSet()->getData()->x_map( pointFlat );
        pointMapped[1] = fracture->getLevelSet()->getData()->y_map( pointFlat );
	
        const scalar_type levelSetValue1 = levelSetOtherFracture->getData()->ylevelSetFunction ( pointMapped );
        ...
        for ( size_type j = 0; j < extendedNumVelocity; ++j )
        {
            ...
            const scalar_type levelSetValue2 = levelSetOtherFracture->getData()->ylevelSetFunction ( pointMapped );
                          ...
                          const size_type& cutRegionFlag )
{
	...
	const scalar_type levelSetValue = levelSetOtherFracture->getData()->ylevelSetFunction ( pointMapped );

...
//FINE MODIFICATO
	...
	/*
	for(int i=0; i<(*M_globalRightHandSide).size(); i++)
	{
		std::cout << "M_globalRightHandSide..." << (*M_globalRightHandSide)[i] << std::endl;
	}
	*/
	...
	*/			  
    ...
    for ( size_type f = 0; f < numberFractures; ++f )
    {
        ...
        for ( size_type i = 0; i < fractureNumberDOFVelocity [ f ]; ++i )
        {
	    	...
	    	P1 [0] =  i*c;
 		    */
            ordinataUncut [ i ] = fracture->getLevelSet()->getData()->y_map(P);
        }
	...
	getfem::interpolation ( fracture->getMeshFEMVelocity(), meshFEMcutFlat, ordinataUncut, ordinataCut );
        ...
        for ( size_type i = 0; i < meshFEMcutFlat.nb_dof(); ++i )
        {
			...
			bgeot::base_node P1 = meshFEMcutFlat.point_of_basic_dof(i);
            P [ 0 ] = fracture->getLevelSet()->getData()->x_map(P1);
            P [ 1 ] = ordinataCut [ i ];

...
LevelSetData::LevelSetData ( const GetPot& dataFile,
			...
			M_map_inv ( dataFile ( ( M_sectionLevelSet + "Map_inv" ).data (), "1" ) ),
            M_map_jac ( dataFile ( ( M_sectionLevelSet + "jacMap" ).data (), "1" ) ),
            M_normal_map ( dataFile ( ( M_sectionLevelSet + "normalMap" ).data (), "1" ) ),
			M_spatialDiscretization( dataFile( (M_section + "domain/" + "spatialDiscretization").data(), 200 ) ),
...
// seconda funzione level set che definisce la frattura . decommentare quello voluto

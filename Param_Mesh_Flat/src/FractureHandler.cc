...
void FractureHandler::init ( )
{
    ...
    M_geometricTransformation = bgeot::geometric_trans_descriptor(
            M_data.getMeshType()); // va a contenere GT_PK(1,1)
    ...
    /*
	 * fractureLength [ 0 ] = M_data.getLengthAbscissa(); //LenghtAbscissa == pezzettino di dominio x
	...
	*/
    ...
    for ( size_type i = 0; i < M_data.getSpaceDimension(); ++i )
    {
        fractureTransformMatrix(i, i)
                = (i < M_data.getSpaceDimension()) ? fractureLength [ i ] : 1.0; // se vero i < M_data.getSpaceDimension() metti fractureLength [ i ] altrimenti 1
    }
	...
	for ( size_type i = 0; i < M_meshFlat.points().size(); ++i )
    {
		...
		base_node node(2);
		node[0]=M_data.x_map(node);
		node[1]=M.data.y_map(node);
	...
	*/
	...
	//Andiamo ad aggiungere il nodo alla mesh
	for ( size_type i = 0; i < M_meshFlat.points().size(); ++i )
    {
        bgeot::base_node P(M_data.getSpaceDimension() + 2);
        bgeot::base_node P1(M_data.getSpaceDimension() + 2);
        P [ 0 ] = M_meshFlat.points() [ i ] [ 0 ]; //P[0] == t;
		
        P1 [ 0 ] = M_levelSet->getData()->x_map(P); //P1[0] == x;
        P1 [ 1 ] =  M_levelSet->getData()->y_map(P);//P1[0] == x;
		
        ind [ i ] = M_meshMapped.add_point(P1);
    }
	
	//andiamo ad aggiungere l'elemento (1d segmento, 2d trangolo etc) alla mesh
...
void FractureHandler::normalVectorAndMap ( const getfem::mesh_fem& mediumMeshFEMPressure )
{
    ...
    for ( size_type i = 0; i < M_meshFEMPressure.nb_dof(); ++i )
    {
        ...
        P1 [ 0 ] = M_levelSet->getData()->x_map(node); //P1[0] == x;
        P1 [ 1 ] =  M_levelSet->getData()->y_map(node);//P1[1] == y;
		
		scalarVector_Type magnificationMapFactor = M_levelSet->getData()->map_jac( P1, dim);
        scalarVector_Type fractureNormal = M_levelSet->getData()->normal_map( P1, dim);
...
size_type FractureHandler::setMeshLevelSetFracture ( FractureHandler& otherFracture, size_type& globalIndex )
{
    ...
    {	
        ...
        for ( size_type d = 0; d < nbDof; ++d )
        {
			...
			base_node node = M_levelSetIntersect [ otherFractureId ]->get_mesh_fem().point_of_basic_dof(d); //Prende la t
            base_node mappedNode ( node.size() +1 );

		    mappedNode [0] =  M_levelSet->getData()->x_map ( node ); 

		    mappedNode [1] =  M_levelSet->getData()->y_map ( node );
        ...
        for ( i_cv << bc_cv; i_cv != size_type(-1); i_cv << bc_cv )
        {
            if (M_meshLevelSetIntersect[ otherFractureId ]->is_convex_cut ( i_cv ) )
            {
				...
				std::cout << " aggiungto   " << i_cv << std::endl;

...
\begin{DoxyCode}
17                                                                              :
18     mf(mf\_), U(mf\_.nb\_basic\_dof()), N(mf\_.linked\_mesh().dim()), gradU(1, N)
19 \{
20     sizes\_.resize(1);
21     sizes\_ [ 0 ] = short\_type(N);
22     mf.extend\_vector(U\_, U);
23 \}
\end{DoxyCode}
...
\begin{DoxyCode}
27 \{
28     size\_type cv = ctx.convex\_num();
29     coeff.resize(mf.nb\_basic\_dof\_of\_element(cv));
30     gmm::copy(gmm::sub\_vector(U,
31             gmm::sub\_index(mf.ind\_basic\_dof\_of\_element(cv))), coeff);
32     ctx.pf()->interpolation\_grad(ctx, coeff, gradU, 1);
33     scalar\_type norm = gmm::vect\_norm2(gmm::mat\_row(gradU, 0));
34     \textcolor{keywordflow}{for} ( size\_type i = 0; i < N; ++i )
35     \{
36         t [ i ] = gradU(0, i) / norm;
37     \}
38 \}
\end{DoxyCode}
...
\begin{DoxyCode}
36     \{
37         \textcolor{keywordflow}{return} sizes\_;
38     \}
\end{DoxyCode}
